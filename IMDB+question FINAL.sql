...
SELECT COUNT(id) AS No_of_movie_from_USA_or_INDIA_in_2019
FROM movie
...
SELECT genre, ROUND(SUM(duration) / COUNT(id)) AS Avg_duration
...
WITH production_company_rank AS (
  SELECT production_company, COUNT(id) AS movie_count, RANK() OVER( ORDER BY COUNT(id) DESC) 
...
SELECT COUNT(m.id) AS Movie_counts
...
top_director AS (
  ...
  INNER JOIN genre g ON g.movie_id = d.movie_id, top_genre 
...
SELECT DISTINCT n.name, COUNT(rt.movie_id) AS movie_count
...
SELECT nm.name AS actor_name, SUM(total_votes), COUNT(m.id) as movie_count, 
...
WITH ratings_def AS (
SELECT title, ROUND( SUM(avg_rating * total_votes)/ SUM(total_votes),2) AS avg_rating 
FROM movie m 
INNER JOIN ratings r ON m.id = r.movie_id 
GROUP BY title
) 
